...
pub fn terminated_statement(input: &mut In<'_>) -> Pr<xulo_core::ast::Statement> {
    let original = *input;
    let s = statement::statement(input)?;
    let has_semicolon = opt_tk(input, Token::Semicolon);
    match s {
        xulo_core::ast::Statement::Expr(es) => {
            Ok(xulo_core::ast::Statement::Expr(xulo_core::ast::ExprStmt {
                expr: es.expr,
