...
for i in 0..<10 {   // 半开区间（排他上界）0…9
  print(i)
}

for i in 0...10 {   // 闭合区间（含上界）0…10
  print(i)
}
```

区间也作为表达式求值：`0..<3` 得 `[0, 1, 2]`，`0...3` 得 `[0, 1, 2, 3]`。`...` 在中缀位置表示闭合区间；列表/对象字面量元素起始位置的 `[...a, b]` / `{ ...x }` 仍是展开运算符。
...
与 JavaScript 对齐，**反引号**（`` ` ``）内的 `` `${expr}` `` 会在运行时求值并拼接，等价于 `str(expr)` 后接 `+`：

```
let name = "Xulo"
print(`Hello, ${name}!`)            // Hello, Xulo!
for i in 1...3 { print(`${i}^2 = ${i * i}`) }
// 1^2 = 1
// 2^2 = 4
// 3^2 = 9
```
...
- **只有反引号字符串会插值**；`"..."` / `'...'` 里的 `${...}` 是普通字符。
- 插值表达式可以是任意表达式：`${ fn(2) }`、`${ {a: 1}.a }`、`${ xs[0] }`、嵌套的 `${ `a${b}` }` 等。
- 反引号字符串可以跨行。
- 转义：`` \` ``（反引号本身）、`\$`（美元符号）、`\${`（字面的 `${`，不插值）、`\\` `\n` `\t` `\r` `\uXXXX` / `\u{...}`。
- 值通过 `str(...)` 转字符串：字符串原样、number/boolean/null 直接格式化、列表/对象按其显示格式输出。
- 插值区域内的 `"` / `'` / 反引号字面量会被整体跳过（其中的 `}` 不会提前结束插值）。
...
**导入解析规则（模块加载器）：**
- 相对路径或本地存在的模块名（如 `./math`、`math`）→ 解析为本地 `.xulo` 文件，参与加载。
- 其余说明符（如 `@xulo/ui`）→ 视为外部包；原生运行时把导入名绑定为 `null` 占位符（仅 `import type` 擦除）。
- `import type` 只供类型检查；运行时仅当导入名同时是运行时值（如 `enum`）时保留其值绑定，以便 `Kind::Admin` 可用，否则完全擦除；`pub enum` 同时导出运行时值与类型。
- 模块按依赖拓扑序加载并执行，导出以导出表方式按名解析；入口文件加载时执行其 `main()`。
- 循环导入（A → B → A）会被拒绝。
...
- `@Effect` 是语言关键字
- 在组件挂载时执行
- 依赖数组前置：`@Effect([id]) fn() { ... }`，变化时重新执行
- 支持返回清理函数
- 清理函数在组件卸载或依赖变化前执行

> 兼容：旧式尾置依赖 `@Effect fn() { ... }, [id]` 仍可解析，但新代码请使用前置写法。

---

## 11.5. 值记忆化（@Memo）（原生计算优化）

```
@Memo([a, b]) let x: number = expensive(a)   // 依赖变化时重算
@Memo([])     let y = compute()              // 空依赖：永久缓存
@Memo         let z = simple()               // 简写：等价 @Memo([])
```

**规则：**
- `@Memo` 是语言关键字
- 只能用在函数体顶层（任意函数，不限于 `View`），禁止嵌套块与模块作用域
- 必须带初值（`@Memo let x` 是错误）
- 依赖数组前置：`@Memo([deps]) let x = expr`；省略或空数组 = 永久缓存
- **原生解释器**：按 (绑定位点, 依赖值) 缓存结果，依赖未变时复用，是计算优化
- **JS 目标**：暂为 no-op（每次渲染重新计算），文档标注
...
| `@Memo` | 任意函数体顶层 | 模块作用域、嵌套块 |

> 闭包（`fn() { ... }`）和嵌套 `fn` 属于独立函数：即使在返回 `View` 的组件函数内部，其体内也不允许使用上述装饰器（`@Memo` 除外，它属于任意函数体顶层）。
...
| 模块加载 | 图加载 + 导出表 | 依赖拓扑序、循环检测、导出符号/类型跨模块校验 |

### 21.5 模块加载（实现要点）

- **加载**：从入口文件开始 DFS，本地导入（相对路径或本地存在的裸名）先加载依赖（后序 → 拓扑序），循环导入报错。
- **语义**：按依赖顺序逐个 `analyze_with`，把依赖导出的符号（真实签名）与类型种子化给导入方；导入不存在的导出名报错。
- **执行**：模块按加载顺序由原生解释器逐个执行，按名把导出值解构绑定给导入方；入口模块执行其 `main()`。
- **外部依赖**：非本地说明符（如 `@xulo/ui`）的导入名绑定为 `null` 占位符（`import type` 完全擦除），无原生实现的值仅在程序真正计算时才失败。
